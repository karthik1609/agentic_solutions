...
graph TD
    ...
    subgraph People
      U[Data Product Team<br/>(BA / PM / Architect / AI Eng)]
    end

    subgraph Platform[AI-First Requirements & Spec Platform (This Repo)]
      S1[Data Gathering<br/>Requirements Service]
      S2[Spec Generator<br/>Service]
    end

    subgraph External
      SRC[Source Knowledge<br/>(Existing Docs, Domain SMEs)]
      CSV[CSV Storage<br/>(Git / Shared Drive)]
      LLM[LLM Providers<br/>(Langdock / OpenAI / Azure / Mock)]
      ODCS[ODCS Standards & Tooling<br/>(Schema, Validators, Downstream DP Platform)]
      EVAL[Evaluation Stack<br/>(Deepeval / Test Harness)]
    end
...
graph TD
    ...
    U[Data Product Team<br/>(CLI user)]

    subgraph DG[Container: data-gathering-requirements]
      DG_CLI[CLI Entry (main.py)]
      DG_APP[Application Services<br/>(RequirementsService, CLIHandler)]
      DG_DOMAIN[Domain Layer<br/>(Models, Ports, Schema Services)]
      DG_ADAPTERS[Adapters<br/>(CSV, LLM, Storage, Evaluation)]
      DG_INF[Infrastructure<br/>(DI Container, Config, Logging)]
    end

    subgraph SG[Container: spec-generator]
      SG_CLI[Typer CLI<br/>(app/cli.py)]
      SG_GEN[Generator Orchestrator<br/>(generator.py)]
      SG_PREFILL[Prefill & Inference<br/>(prefill.py, field/metadata_inference.py)]
      SG_PLANNER[Planner & Prompts<br/>(planner.py, prompts/)]
      SG_LLM[LLM Client / Mock<br/>(llm.py, mock_generator.py)]
      SG_POST[Sanitizer & Validator<br/>(sanitizer.py, validator.py, schema_utils.py)]
      SG_ANN[Source Tracking & Annotation<br/>(yaml_annotator.py, source_tracker.py)]
      SG_IO[IO & Config<br/>(io.py, config_loader.py, odcs_config.py)]
    end
...
graph TB
    ...
    subgraph Entry
      MAIN[main.py]
      CLI_HANDLER[CLIHandler<br/>(application/services/cli_handler.py)]
    end

    subgraph AppLayer[Application Layer]
      REQ_SERVICE[RequirementsService<br/>(application/services/requirements_service.py)]
    end

    subgraph Domain[Domain Layer]
      MODELS[Models<br/>(RequirementsDocument,<br/>EnhancementData,<br/>ProcessingResult,<br/>ValidationResult)]
      PORTS[Ports (Interfaces)<br/>(LLMProvider, DataSource,<br/>DataSink, StorageProvider,<br/>RequirementsRepository,<br/>MetadataRepository, EvaluatorPort)]
      SCHEMA_SVC[Schema & Validation Services<br/>(schema_repository, schema_validator)]
      UTILS[Helpers<br/>(TextNormalizer,<br/>FewShotExamples,<br/>ContextFileResolver)]
    end

    subgraph Adapters[Adapters Layer]
      subgraph DataAdapters
        CSV_SRC[CSVDataSource<br/>(csv_data_source.py)]
        CSV_SINK[CSVDataSink<br/>(csv_data_sink.py)]
        CSV_REPO[CSVRequirementsRepository<br/>(csv_requirements_repository.py)]
      end
    ...
    subgraph Infra[Infrastructure]
      DI_CONTAINER[DI Container<br/>(infrastructure/container.py)]
      CFG[Config<br/>(config.py)]
      LOG[Logging<br/>(logging_config.py)]
      TOKENS[TokenTracker<br/>(token_tracker.py)]
    end
...
graph TB
    ...
    subgraph CLI[CLI Layer]
      TYPER[Typer CLI<br/>(cli.py)]
    end

    subgraph Orchestration
      GEN[Generator<br/>(generator.py)]
      VALIDATE_CMD[validate_spec command<br/>(validate_spec.py)]
      INPUT_VALIDATE[Input Completeness<br/>(input_validator.py)]
    end

    subgraph PrefillLayer[Prefill & Inference]
      PREFILL[prefill.py<br/>(CSV → skeleton prefill)]
      PREFILL_ISO[prefill_isolate.py<br/>(focused prefill ops)]
      FIELD_INF[field_inference.py<br/>(column-level inference)]
      META_INF[metadata_inference.py<br/>(dataset metadata inference)]
    ...
    subgraph Prompting
      PLANNER[planner.py<br/>(prompt builder)]
      PROMPTS[prompts/*<br/>(domain, persona, instructions)]
    end

    subgraph LLMBlock[LLM / Mock]
      LLM_CLIENT[LLMClient<br/>(llm.py)]
      MOCK_GEN[MockGenerator<br/>(mock_generator.py)]
    end

    subgraph PostProcess[Sanitize / Validate / Annotate]
      SANITIZER[sanitizer.py]
      SCHEMA_UTILS[schema_utils.py<br/>(JSON Schema → skeleton)]
    ...
    subgraph IOConfig[IO & Config]
      IO[io.py<br/>(CSV IO, JSON helpers)]
...
sequenceDiagram
    ...
    participant DG_APP as DG Application Layer<br/>(CLIHandler, RequirementsService)
    participant DG_DOM as DG Domain & Ports
    participant DG_AD as DG Adapters<br/>(CSV, LLM, Storage)
    participant FS as File System<br/>(source_files,<br/>prefilled_files,<br/>enhanced_results,<br/>results)
    participant LLM as LLM Provider<br/>(Langdock / OpenAI / Azure / Mock)
    ...
    participant ODCS as ODCS Consumer<br/>(Downstream DP Platform / Human Reviewer)
